    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
      ...
      <Card className="p-6">
        ...
        <div className="mt-6 p-3 bg-muted/50 rounded-lg">
          <p className="text-xs text-muted-foreground">
            <strong className="text-foreground">Insight:</strong> Gen Z drives 65% of beauty trend adoption, with
            highest engagement on TikTok and Instagram Reels.
          </p>
        </div>
      </Card>
          ...
          {geographicData.map((region, index) => (
            <div key={index} className="flex items-center justify-between p-3 rounded-lg border border-border">
              <div>
                ...
                <div className="text-sm text-muted-foreground">{region.trends} active trends</div>
              </div>
              <Badge variant={region.growth.startsWith("+") ? "default" : "secondary"}>{region.growth}</Badge>
            </div>
          ))}
        </div>
      </Card>
      ...
      <Card className="p-6 lg:col-span-2 xl:col-span-1">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold text-foreground">L'Oréal Brand Mapping</h3>
        </div>
          ...
          {productMapping.map((item, index) => (
            <div key={index} className="p-4 rounded-lg border border-border bg-card">
              <div className="flex items-center justify-between mb-2">
                ...
                <Badge
                  variant={
                    item.opportunity === "High" ? "default" : item.opportunity === "Medium" ? "secondary" : "outline"
                  }
                >
                  {item.opportunity}
                </Badge>
      ...
      <Card className="p-6 lg:col-span-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 rounded-lg border border-border bg-muted/30">
            <h4 className="font-medium text-foreground mb-2">Trend Response Speed</h4>
          ...
          <div className="p-4 rounded-lg border border-border bg-muted/30">
            <h4 className="font-medium text-foreground mb-2">Engagement Success Rate</h4>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
